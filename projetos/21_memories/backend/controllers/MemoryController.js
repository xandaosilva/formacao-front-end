...
const createMemory = async(req, res) => {
    try {
        ...
        const newMemory = new Memory({ title, src, description });
...
const toggleFavorite = async(req, res) => {
    try {
        ...
        await memory.save();
        res.json({ msg: "Memória adicionada aos favoritos.", memory });
    } catch (error) {
        console.log(error.message);
        res.status(500).send("Ocorreu um erro.");
    }
...
module.exports = { createMemory, getMemories, getMemory, deleteMemory, updateMemory, toggleFavorite, addComment };
